...
export default function searchReducer(state = initialState, action) {
  switch(action.type) {
    ...
    case SEARCH: {
      ...
      //val.includes(value)
      const serachedData = state.contents.filter((val) => filteredData.find(({ccy,lowerLimit,upperLimit})=>(val.ccy === ccy && val.invoice_amount>lowerLimit && val.invoice_amount<upperLimit)));
